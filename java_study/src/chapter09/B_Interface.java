...
interface PlayingCard {
	...
	public abstract String getCardNumber(); // public abstract 생략 가능
	String getCardKind();
	
	// 3. 디폴드 메서드
	// : 인터페이스에 새 기능을 추가하면서 기존 구현을 포함
	// >> 기본 구현을 제공, 오버라이딩(재정의) 가능
...
class Card implements PlayingCard {
	...
	public void defaultMethod() {
		System.out.println("Card 클래스에서 재정의된 디폴드 메서드입니다.");
	}
}
...
public class B_Interface {
	public static void main(String[] args) {
		...
		// 재정의(오버라이딩)가 선택적
		card.defaultMethod(); // Card 클래스에서 재정의된 디폴드 메서드입니다.
